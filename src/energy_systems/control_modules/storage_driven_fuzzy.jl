...
function upper_plr_limit(mod::CM_StorageDrivenFuzzy, sim_params::Dict{String,Any})::Float64
    ...
    return mod.state_machine.state == 2 ? mod.parameters["plr_limit"] : 0.0
end
...
function run_fuzzy!(mod_params::Dict{String,Any}, sim_params::Dict{String,Any})    
    ...
    plr_diff, SOC_target = fuzzy_control(p_now, p_future, p_stability, SOC)
...
function fuzzy_control(p_now, p_trend, p_volatility, SOC_now)
      low = max(min((p_now - -137) / 1, 1, (80 - p_now) / 25), 0)
      med = max(min((p_now - 55) / 25, (100 - p_now) / 20), 0)
      high = max(min((p_now - 80) / 20, 1, (1001 - p_now) / 1), 0)
      falling = max(min((p_trend - -258) / 1, 1, (6 - p_trend) / 13), 0)
      rising = max(min((p_trend - -7) / 13, 1, (247 - p_trend) / 1), 0)
      low = max(min((p_volatility - -1) / 1, 1, (15 - p_volatility) / 11), 0)
      high = max(min((p_volatility - 4) / 11, 1, (266 - p_volatility) / 1), 0)
      ...
      ant2 = min(low, min(falling, min(low, high)))
      ant3 = min(low, min(falling, min(high, low)))
      ant4 = min(low, min(falling, min(high, high)))
      ant5 = min(low, min(rising, min(low, low)))
      ant6 = min(low, min(rising, min(low, high)))
      ant7 = min(low, min(rising, min(high, low)))
      ant8 = min(low, min(rising, min(high, high)))
      ant9 = min(med, min(falling, min(low, low)))
      ant10 = min(med, min(falling, min(low, high)))
      ant11 = min(med, min(falling, min(high, low)))
      ant12 = min(med, min(falling, min(high, high)))
      ant13 = min(med, min(rising, min(low, low)))
      ant14 = min(med, min(rising, min(low, high)))
      ant15 = min(med, min(rising, min(high, low)))
      ant16 = min(med, min(rising, min(high, high)))
      ant17 = min(high, min(falling, min(low, low)))
      ant18 = min(high, min(falling, min(low, high)))
      ant19 = min(high, min(falling, min(high, low)))
      ant20 = min(high, min(falling, min(high, high)))
      ant21 = min(high, min(rising, min(low, low)))
      ant22 = min(high, min(rising, min(low, high)))
      ant23 = min(high, min(rising, min(high, low)))
      ant24 = min(high, min(rising, min(high, high)))
      ...
      @inbounds for (i, x) = enumerate(P2H_agg)
              ...
              P2H_agg[i] = max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(min(ant1, keep), min(ant2, keep)), min(ant3, rise)), min(ant4, rise)), min(ant5, rise)), min(ant6, rise)), min(ant7, rise)), min(ant8, rise)), min(ant9, keep)), min(ant10, lower)), min(ant11, keep)), min(ant12, keep)), min(ant13, rise)), min(ant14, rise)), min(ant15, rise)), min(ant16, rise)), min(ant17, lower)), min(ant18, lower)), min(ant19, lower)), min(ant20, lower)), min(ant21, keep)), min(ant22, rise)), min(ant23, keep)), min(ant24, keep))
          end
