...
function control(
    ...
    # for fixed input/output temperatures, overwrite the interface with those. because we
    # have specific temperatures, we set the minimum and maximum to the same value
...
function check_heat_out(
    ...
    if unit.controller.parameters["consider_m_heat_out"] == true
        if (
            unit.output_interfaces[unit.m_heat_out].target.sys_function == sf_transformer
...
function calculate_energies_heatpump(unit::HeatPump,
    ...
    while (
        ...
        if unit.input_temperature === nothing
            ...
            if current_in_temp === nothing && unit.constant_cop === nothing
                @error "Error: The input temperature for $(unit.uac) could not be detected. " *
                    "Please specify one with the parameter 'input_temperature' or check the " *
        ...
        if unit.output_temperature === nothing
            ...
            if current_out_temp === nothing && unit.constant_cop === nothing
                @error "Error: The output temperature for $(unit.uac) could not be detected. " *
                    "Please specify one with the parameter 'output_temperature' or check the " *
                    "connected components."
                throw(InputError)
            end
        else
        ...
        if current_in_temp >= current_out_temp && unit.constant_cop === nothing
            ...
            # the usage_fraction calculation? Or should we deny the bypass and force users
            # to implement heat pumps in parallel if they want a bypass?
            ...
            cop = unit.constant_cop === nothing ?
                ...
                unit.constant_cop
            if cop === nothing
                @error ("Input and/or output temperature for heatpump $(unit.uac) is not given. " *
                    "Provide temperatures or fixed cop.")
                throw(InputError)
            end
...
function calculate_energies(
    ...
    if heat_in_has_inf_energy && heat_out_has_inf_energy
        ...
        @warn "The heat pump $(unit.uac) has unknown energies in both its inputs and outputs. " *
            "This cannot be resolved. Please check the order of operation and make sure that " *
            "either the inputs or the outputs have been fully calculated before the heat pump " *
            "$(unit.uac) has its potential step."
