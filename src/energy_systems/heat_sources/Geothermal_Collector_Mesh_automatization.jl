...
function create_mesh_y(min_mesh_width,max_mesh_width, expansion_factor,pipe_laying_depth,Pipe_Diameter_outer, total_depth_simulation_domain)
    ...
    for i in 1:Int(floor((segment_array_y[2] - segment_array_y[1]) / min_mesh_width))
        ...
        elseif sum(dy_1[1:i]) + min(dy_1[i-1] * expansion_factor, max_mesh_width) < (segment_array_y[2] - segment_array_y[1]) / 2
        ...
        elseif sum(dy_1[1:i]) + min(dy_1[i-1] * expansion_factor, max_mesh_width) >= (segment_array_y[2] - segment_array_y[1]) / 2
            ...
            # if turning point is already reached, the size of the next element decreases according to the defined expansion expansion_factor
    ...
    for j = 1: Int(floor((segment_array_y[4] - segment_array_y[3]) / min_mesh_width))
        ...
        # as long as the depth (sum of dy_1) in the loop iteration stays smaller than the half segment size,
        # the next element of dy increases (according to the defined expansion factor)
        elseif sum(dy_3[1:j]) + min(dy_3[j-1] * expansion_factor, max_mesh_width) < (segment_array_y[4] - segment_array_y[3]) / 2
            difference *= expansion_factor    
...
function create_mesh_x(min_mesh_width, max_mesh_width, expansion_factor, Pipe_Diameter_outer)
    ...
    for i in 1:Int(floor((segment_array_x[3] - segment_array_x[2]) / min_mesh_width))
        ...
        elseif sum(dx_2[1:i]) + min(dx_2[i-1] * expansion_factor, max_mesh_width) < (segment_array_x[3] - segment_array_x[2])
            difference *= expansion_factor
