...
mutable struct FlexibleSink <: Component
    ...
    function FlexibleSink(uac::String, config::Dict{String,Any}, sim_params::Dict{String,Any})
        ...
        for (param_name, param_def) in FLEXIBLE_SINK_PARAMETERS
            try
                extracted_params[param_name] = extract_parameter(FlexibleSink, config, param_name,
        ...
        try
            ...
            validate_config(FlexibleSink, config, extracted_params, uac, sim_params)
        catch e
            @error "$(sprint(showerror, e))"
            constructor_errored = true
        end
        ...
        init_values = init_from_params(FlexibleSink, uac, extracted_params, config, sim_params)
