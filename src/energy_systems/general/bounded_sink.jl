...
mutable struct BoundedSink <: Component
    ...
    function BoundedSink(uac::String, config::Dict{String,Any}, parameters::Dict{String,Any})
        ...
        elseif haskey(config, "use_ambient_temperature_from_weather_file") && config["use_ambient_temperature_from_weather_file"] && haskey(parameters, "weatherdata") 
            ...
            # println("Info: For bounded sink '$uac', the temperature profile is taken from the ambient temperature of the project-wide weather file.")
        else
            temperature_profile = nothing
            # println("Info: For bounded sink '$uac', no temperature is set.")
        end
